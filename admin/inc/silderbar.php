<?php 
  ...
  //获取session箱子内的用户登录信息
  //因为当前页面是被index.php载入执行的，所以路径是相对于index而言，要想彻底根治这个问题
  //需使用物理路径+/../..的方式
...
<div class="aside">
    ...
    <ul class="nav">
      ...
      <?php $menu_post = array('posts','post-add','category') ?>
      <li <?php echo in_array($current_page,$menu_post ) ? 'class="active"' : "" ?>>
        ...
        <ul id="menu-posts" <?php echo in_array($current_page,$menu_post ) ? 'class="collapse in"' : 'class="collapse"' ?>>
          ...
          <li <?php echo $current_page === 'category' ? 'class="active"' : ""; ?>><a href="/admin/categories.php">分类目录</a></li>
        </ul>
      </li>
      ...
      <li  <?php echo in_array($current_page,$menu_settings ) ? 'class="active"' : "" ?>>
        ...
        <ul id="menu-settings" <?php echo in_array($current_page,$menu_settings ) ? 'class="collapse in"' : 'class="collapse"' ?>>
